...
//这个start的作用是防止重复，若测试用例为{2,3,6,7},target=7;若加上start,结果为[[2,2,3],[7]],若不加start,结果为[[2,2,3],[2,3,2],[3,2,2],[7]]
//不加start就会出现很多重复，具体原因：例如2,2,3 当结果为2,2,3时,此时再进行下去，为2,3...由于start为1，所以不可能再取到2，因此
//也就不可能存在2,3,2的这种情况，3,2,2同理。
